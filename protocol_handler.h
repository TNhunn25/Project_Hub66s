                ...
                uint8_t opcode, const JsonVariant &data, unsigned long packetTime, const String &recvAuth)
{
    ...
    case CONFIG_DEVICE:
    {
        ...
        int new_id = licData["id"].as<int>(); // Mặc định lấy config_id
        ...
        if (new_lid > 0 && new_id > 0)
        {
            ...
            Serial.println("✅ Cấu hình thành công: LID = " + String(new_lid) + ", ID = " + String(new_id));
            respDoc["status"] = 0;
            respDoc["lid"] = globalLicense.lid;
    ...
    case LIC_INFO:
    {
        // Trả về thông tin cấu hình hiện tại của node để sender cập nhật
        StaticJsonDocument<256> respDoc;
